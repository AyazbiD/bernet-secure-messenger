...
This module provides a robust implementation of Hybrid Encryption,
combining the security of RSA-2048 with the efficiency of AES-256-GCM.
It handles key generation, encryption, decryption, and secure storage of keys.
"""
import os
import base64
import hashlib
...
def generate_rsa_keypair():
    """
    Generates a new RSA-2048 key pair for asymmetric encryption.
    
    Returns:
        tuple: A tuple containing (private_key_pem, public_key_pem) encoded in PEM format.
    """
...
def derive_key_from_pin(pin: str, salt: bytes) -> bytes:
    """
    Derives a cryptographically secure 256-bit AES key from a user-provided PIN
    using the PBKDF2-HMAC-SHA256 key derivation function.
    
    Args:
        pin (str): The user's personal identification number.
        salt (bytes): A cryptographic salt to protect against rainbow table attacks.
        
    Returns:
        bytes: A derived 32-byte (256-bit) AES key.
    """
    kdf = PBKDF2HMAC(
...
def encrypt_private_key_with_pin(private_key_pem: bytes, pin: str) -> dict:
    # encrypt private key with pin via aes-gcm
    salt = os.urandom(16)
    iv = os.urandom(12)
...
def encrypt_message_hybrid(plaintext: str, recipient_public_key_pem: bytes) -> dict:
    """
    Performs Hybrid Encryption on a plaintext message.
    1. Generates an ephemeral AES-256 key and encrypts the plaintext using AES-GCM.
    2. Encrypts the ephemeral AES key using the recipient's public RSA key (OAEP padding).
    
    Args:
        plaintext (str): The message to encrypt.
        recipient_public_key_pem (bytes): The recipient's public RSA key.
        
    Returns:
        dict: The hybrid encrypted payload containing base64-encoded ciphertext, IV, and the RSA-encrypted AES key.
    """
    ...
    # Generate ephemeral AES-256 key (32 bytes)
    ...
    rsa_encrypted_aes_key_recipient = public_key.encrypt(
        aes_key,
        padding.OAEP(
    ...
    return {
        ...
        'encrypted_key_b64': base64.b64encode(rsa_encrypted_aes_key_recipient).decode('utf-8'),
...
def decrypt_message_hybrid(ciphertext_b64: str, iv_b64: str, encrypted_key_b64: str, my_private_key_pem: bytes) -> str:
    ...
    1. Decrypts the ephemeral AES key using the user's private RSA key.
    2. Decrypts the ciphertext using the decrypted AES-256-GCM key.
    
    Args:
        ciphertext_b64 (str): Base64-encoded AES ciphertext.
        iv_b64 (str): Base64-encoded initialization vector.
        encrypted_key_b64 (str): Base64-encoded AES key (encrypted with RSA).
        my_private_key_pem (bytes): The recipient's private RSA key.
    ...
    rsa_encrypted_aes_key_recipient = base64.b64decode(encrypted_key_b64)
